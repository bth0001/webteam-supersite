...
router.get("/", function (req, res, next) {
  ...
  if (req.query.search) {
    ...
    db.User.find({ firstName: regex })
      .collation({ locale: 'en', caseLevel: false, caseFirst: "off" }).sort({ isMaster: -1 })
    ...
    db.User.find({}).collation({ locale: 'en', caseLevel: false, caseFirst: "off" }).sort({ isMaster: -1 }).skip((perPage * pageNumber) - perPage).limit(perPage).exec(function (err, foundUsers) {
...
router.delete("/:id", function (req, res) {
  db.User.findByIdAndRemove(req.params.id, function (err, foundUser) {
    if (err) {
      ...
      req.flash("success", `${foundUser.firstName}'s account has been deleted.`)
      res.redirect("/accounts");
    }
  })
})
